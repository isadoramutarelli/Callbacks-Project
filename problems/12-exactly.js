...
/* function exactly(array, number, callback) {
    // Filter the array based on the callback
    const filteredArray = array.filter(callback);

    // Check if the number of elements in the filtered array matches the specified number
    return filteredArray.length === number;
  } */
